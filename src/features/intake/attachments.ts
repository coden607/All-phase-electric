...
export async function storeLeadAttachments(leadId: string, files: File[]) {
  ...
  for (const file of files) {
    ...
    const upload = await supabase.storage.from('lead-attachments').upload(path, bytes, { contentType: file.type, upsert: false });
    if (upload.error) throw new Error(upload.error.message);
    const meta = await supabase.from('lead_attachments').insert({ lead_id: leadId, storage_path: path, original_filename: file.name, mime_type: file.type, byte_size: file.size });
    if (meta.error) { await supabase.storage.from('lead-attachments').remove([path]); throw new Error(meta.error.message); }
  }
}
